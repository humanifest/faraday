...
value-domain digest, and to conserve the receipt row count before a handoff can
draft or record a canonical run, so a post-run receipt edit cannot delete or
reshape the source-domain check.
...
Free-text answers, semicolon-list commitment boundaries, and measurement
parameter bindings are preserved for the scaffold audit; the interview uses
trimming only for blank detection, menu choices, and ordinary spaces adjacent to
semicolon separators. Padded commitments therefore become visible blocking
findings instead of silently cleaner draft text.
...
and padded questions block readiness. Deterministic synthesis reports the counts
of still-open and deferred questions and labels them as live ambiguity, not
evidence, answers, or authorization to prefer an explanation. Rigor audit now
emits a warning for still-open or deferred canonical questions, preserving
unresolved ambiguity as a quality signal without making unfinished inquiry
structurally invalid. Canonical questions can also be deferred with
`question defer --rationale`; deferral requires nonempty rationale prose, remains
unresolved in rigor and synthesis, and cannot overwrite an answered question.
Guided briefs and the provider-free
interview can also preserve typed claim-level boundary proposals in
...
rewritten into support for adaptation, mechanism, attribution, or intent. The
...
diagnostics, canary assessments, or retained acquisition/conformance records;
skipped means unperformed, not a place to park observations under an invalid
run. Structured measurement-validity metadata is also replayed for every
performed validity gate: each frozen check must have exactly one result, the
evidence type must match the protocol, passed/warning/failed
...
observation dataset, declared run output, and retained body digest before
package gates may cite the adjudication body. When a structured
...
treating them as proof of adaptation, mechanism, attribution, or intent.
Structured
...
Status, evidence, event, predecessor, reason, review-artifact locator/root,
creator, and retained conclusion-ceiling handles must be canonical without
surrounding whitespace. Retained status reasons and conclusion ceilings also use
the bounded report-language guard, so overclaim terms cannot turn an
artifact-backed correction into stronger scientific authority. Every later event
must name the exact latest event with
...
always replaces dataset artifact locators and normalizes every recursively nested
typed location-metadata value—whether supplied as a scalar, list, or object—to
the one canonical scalar marker. Artifact records expose exactly `locator`,
`sha256`, `size_bytes`, `media_type`, and `metadata`; hashes, roles, media types,
and sizes remain available without inventing a basename as false provenance.
Other structured locator and path fields may retain only canonical relative
logical values: URI/drive forms, platform-root segments, `.local` host segments,
traversal, percent encodings, tilde/backslash forms, and controls fail closed.
The explicit identity keys
...
For prospective analysis of whether separately sourced objects form a required
directed dependency graph, use the public-development
[source-composability evaluator](docs/source-composability.md). It verifies
...
bias-assessment artifact's independent-review flag, non-evidence,
non-conclusion, non-publication boundary, retained limitations, and
...
rewritten bias summary into the terminal review chain. It also replays each
...
pinned extraction trail. Citation rationales and retained
citation-verification limitations also reject report-prohibited overclaiming
language, so a source-support check cannot claim proof, confirmation,
validation, or explanation beyond the bounded verdict. It also replays each
extraction source-review envelope:
...
whenever effects are reproducibly derived from source-reported arm summaries.
...
coverage checks. Verification rationales and retained effect-verification
limitations also reject report-prohibited overclaiming language, so an
independent arithmetic/transcription check cannot claim proof, confirmation,
...
timing counts, status, retained limitation prose, and non-authority fields must
all remain internally consistent. Departure commitments, actual methods, reasons,
impact assessments, corrective actions, and retained deviation limitations are
bounded report prose as well as canonical text: they can disclose uncertainty
and corrective work, but cannot claim proof, confirmation, validation, or
explanation. Downstream qualitative synthesis and quantitative pooling replay
the embedded departure rows and frozen-plan snapshot again, so a copied
synthesis or pooling artifact cannot drop, pad, or recategorize departure
history, or rewrite the synthesis question, conclusion rule, effect measure,
contrast, or
...
padded review receipts, decision fields, artifact handles, duplicate
conditional-approval obligations, and padded obligations, and rejects malformed
non-lowercase or non-hex review artifact digests before local-byte verification
is even staged.
...
is right, expected result under the exact registered null model or one
registered competing model, and the observation that would weaken it. The
...
The competing-model reference is retained and replayed against the hypothesis
