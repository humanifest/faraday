...
reviewed `measurement_id`. The typed checks survive serialization and are part
...
Stable transformation, calibration, gate, and derived-observation IDs must be
unique after trimming whitespace, so cosmetic padding cannot split one custody
node into apparently separate provenance records.
