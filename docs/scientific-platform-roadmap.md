...
declared context kind. Active and `pending_review` hypotheses are exposed in
separate context lanes, and pending hypotheses retain their unapproved workflow
state when cited for review. Context and proposal purpose bindings, context
handles, proposal IDs, generator provider/model handles, suggestion IDs,
evidence references, review IDs, reviewer handles, and review-decision
suggestion IDs must be canonical without surrounding whitespace before coverage
or duplicate checks, so whitespace padding cannot be silently rewritten into a
task purpose, citation, source identity, or review obligation. The resulting
