...
canonical service applies its stricter normalized and run-bound checks. It also
requires a nonblank `analysis_id` when evidence is not backed by a recorded run;
...
linear sequence. Retained status reasons and conclusion ceilings now also use
the bounded report-language guard on write and authoritative replay, preventing
a stored correction event from being rewritten into overclaiming scientific
authority.
...
It preserves raw free-text answers, semicolon-list commitment boundaries, and
`name=value` measurement-parameter bindings for the scaffold audit, using
...
Guided alias/proxy delivery: JSON design briefs and the provider-free interview
now accept the canonical `alias_proxy_commitment` shape on the primary
measurement and on secondary, control, or causal measurement entries. The
scaffold validates concealment scope, public-label binding, private mapping
SHA-256, bounded rationale, limitations, reveal conditions, and proxy-construct
rules before emitting a review-only `alias-proxy-commitments-draft.json`
artifact. A proxy primary measurement also requires at least one prospective
measurement-validity check, so the hidden construct cannot enter design review
as an unexplained substitute for the public observable. The artifact retains
hashes and public labels only; it does not reveal hidden entities, prove proxy
...
requires one exact ordered structured definition and one exact ordered
reproducible measurement definition in addition to its control-family
rationale. Dataset-column controls receive typed domain
...
checks, and redacted root policy. Conditional-discharge replay now also compares
the retained discharge source with the verification receipt, preserving
compliance statuses, active-control horizons, cited evidence digests, exact
locations, JSON selected-value hashes, and the monitoring flag. These checks
prevent an internally hash-consistent package from erasing or strengthening
protected-dataset verification metadata, while still refusing to treat redacted
roots as current local byte access. Protocol-deviation
...
alongside the retained scientific constraints and reference index. The context
scientific constraints now also preserve the acquisition-timing boundary:
frozen sensor, stream, clock, and control-window commitments may guide review,
but they remain design provenance rather than proof of custody, calibration,
synchronization, or timing validity. Older records without the retained boundary
remain visible as `legacy_missing` rather than being silently upgraded. The
...
prefix must match its declared context kind. Evidence correction, qualification,
...
mechanism, legal characterization, or replication beyond recorded evidence or
authorizing collection, protocol freeze, data registration, evidence recording,
or other canonical action; proposal validation replays them from the hash-checked context file, and
...
connector, add-on, experiment, import, attestation, or fixture route into source
truth, custody, or evidence eligibility. Evidence admission now replays every run
dataset even for non-scientific exploratory records that omit a single dataset
selector. Run intake and scientific-evidence admission replay also check
...
body and recomputes the retained `selected_value_sha256` for the exact selected
JSON value, so file-level package integrity cannot hide a fabricated or drifted
canary evidence location.
...
custody, or ethics verification state while preserving a fresh manifest hash.
For conditional human-subject approvals, package verification also cross-checks
the retained discharge and verification receipt so condition outcomes, horizons,
evidence locations, and monitoring obligations cannot be rewritten independently
inside an otherwise hash-consistent export. A shared application integrity check
